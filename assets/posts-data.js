window.BLOG_POSTS = [
  {
    "title": "Android开发入门",
    "url": "post-android-intro.html",
    "date": "2026-08-19",
    "categories": [
      "Android"
    ],
    "tags": [
      "Android入门",
      "APK",
      "Java"
    ],
    "summary": "从 Android Studio 环境配置、APK 项目目录，到使用 XML 与 Java 完成图片切换应用并打包 APK 的完整入门记录。"
  },
  {
    "title": "初识ZIP文件结构",
    "url": "post-zip.html",
    "date": "2026-07-26",
    "categories": [
      "CTF"
    ],
    "tags": [],
    "summary": "参考： https://goodapple.top/archives/700 （很棒的基础教学博客） zip文件又三个部分组成： 压缩源文件数据区+压缩源文件目录区+压缩源文件目录结束标志 背景：创建两个zip文件 一个..."
  },
  {
    "title": "初识PE文件结构",
    "url": "post-20260709-pe-file-structure.html",
    "date": "2026-07-09",
    "categories": [
      "Reverse_Wiki"
    ],
    "tags": [
      "Reverse_Wiki",
      "PE",
      "Windows",
      "逆向"
    ],
    "summary": "PE 文件结构入门笔记，整理 PE 工具、PE 指纹、DOS 头、PE 文件头、节表与节数据定位。"
  },
  {
    "title": "UniCTF（自解re 纯菜轻喷）",
    "url": "post-20260709-unictf-re.html",
    "date": "2026-07-09",
    "categories": [
      "CTF"
    ],
    "tags": [
      "CTF",
      "逆向",
      "RE",
      "UniCTF"
    ],
    "summary": "UniCTF 个人 RE 自解记录，整理 c_sm4、r_png、Unity IL2CPP、wasm 逆向等题目的分析和 exp 思路。"
  },
  {
    "title": "SMC（极客2025为例）",
    "url": "post-20260708-geek2025-smc.html",
    "date": "2026-07-08",
    "categories": [
      "CTF"
    ],
    "tags": [
      "CTF",
      "逆向",
      "SMC",
      "极客2025"
    ],
    "summary": "以极客 2025 SMC 题为例，记录自修改代码、VirtualProtect、动态调试与 SMC 重新分析思路。"
  },
  {
    "title": "手脱压缩壳（IS2025 xtea为例）",
    "url": "post-20260708-is2025-xtea-unpack.html",
    "date": "2026-07-08",
    "categories": [
      "CTF"
    ],
    "tags": [
      "CTF",
      "逆向",
      "脱壳",
      "XTEA"
    ],
    "summary": "以 IS2025 xtea 为例，记录压缩壳与加密壳、ESP 定律、OEP 寻找、Dump 与 IAT 修复流程。"
  },
  {
    "title": "汇编",
    "url": "post-20260708-assembly.html",
    "date": "2026-07-08",
    "categories": [
      "语言"
    ],
    "tags": [
      "汇编",
      "Reverse"
    ],
    "summary": "汇编基础笔记，整理计算机架构、寄存器、栈结构、x32dbg、FS/TEB、PEB 与常见汇编指令。"
  },
  {
    "title": "Python",
    "url": "post-20260708-python.html",
    "date": "2026-07-08",
    "categories": [
      "语言"
    ],
    "tags": [
      "Python"
    ],
    "summary": "Python print、input、变量、数据类型、列表、函数、面向对象与文件操作笔记。"
  },
  {
    "title": "C语言进阶",
    "url": "post-20260707-fe9225.html",
    "date": "2026-07-07",
    "categories": [
      "语言"
    ],
    "tags": [
      "C"
    ],
    "summary": "C 语言函数、指针、结构体与动态内存重点整理。"
  }
];
